-- ── STRICT per-location consumption (reverse 043's cross-location absorb) ──
-- Migration 043 made record_waste / confirm_daily_sales / execute_production_run
-- absorb any shortfall via a "deficit batch" that fell back to ANY location and
-- drove it negative. That silently consumed stock from other stations (e.g. a
-- kitchen sale draining the bar / warehouse), blurring per-location accounting.
--
-- New rule (owner decision — strict + clear transfer): after FIFO at the routed
-- station leaves a shortfall, if the ingredient still has active stock at ANOTHER
-- location, RAISE `location_short:<loc>:<ingredient>` so the app can prompt a
-- transfer. Only when NO other location holds the ingredient do we absorb the
-- shortfall as negative at the routed station — preserving the deliberate
-- negative-stock / true-oversell behaviour. Single-station tenants are
-- unaffected (no "other location" ever exists). The no-batch (count-only)
...
begin
  ...
  loop
    ...
    if remaining > 1e-9 then
      -- Strict: stock exists at another station ⇒ refuse so the UI prompts a transfer.
      if exists (
        select 1 from public.ingredient_batches
        where tenant_id = v_tenant and ingredient_id = rec.ingredient_id
      ...
      -- Nothing elsewhere ⇒ true oversell: absorb as negative at the routed station.
